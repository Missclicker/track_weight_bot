...
3. Free tier as of September 2026: roughly 15 requests/min and ~1,500 requests/day on Flash / Flash‑Lite models, image input included. **Do not enable billing on that project** unless you want to pay — the free tier applies only while billing is off. When a limit is hit the bot does not retry: it sets that one model aside (until the next minute, or until the daily counters reset at midnight US Pacific) and says so in the chat — "Ліміт AI на фото вичерпано на сьогодні. Опиши їжу текстом: /їжа ..." — so the group loses photo estimates while `/їжа <текст>`, `/спорт`, corrections and the weekly report keep working on the text model.
